...
function wpeSideNav() {
  ...
  array(
    ...
    'link_before'     => '',
    'link_after'      => '',
    'items_wrap'      => '<ul class="rk-menu rk-categories-menu">%3$s</ul>',
    'depth'           => 0,
    'walker'          => ''
    )
  );
...
function register_html5_menu() {
  register_nav_menus(array(
    ...
    'sidebar-menu' => __('Меню в портфолио', 'wpeasy'),
...
function html5blankcomments($comment, $args, $depth) {
  ...
  if ( 'div' == $args['style'] ) {
    $tag = 'div';
    $add_below = 'comment';
    // falback, TODO - add cheking author or not in this comment tread
    $author = '<small class="ae-u-regular">&nbsp; (Author)</small>';
  } else {
    $tag = 'li';
...
?>
  ...
  <article class="rk-comment">
    <aside class="rk-comment__avatar"><?php if ($args['avatar_size'] != 0) echo get_avatar( $comment, $args['63'] ); ?></aside>
      <div class="rk-comment__content">
        <header class="rk-comment__header">
          <h5 class="rk-comment__author ae-u-bolder"><?php printf(__('%s'), get_comment_author_link()) ?><?php echo $author; ?></h5>
          <div class="rk-comment__meta">
            <p class="ae-kappa">&nbsp; <?php printf( __('at %2$s %1$s'), get_comment_date('F jS, Y'),  get_comment_time('g:i a')) ?></a></p>
          </div>
        </header>
        <div class="rk-comment__body">
          <?php comment_text() ?>
        </div>
        <div class="rk-comment__actions ae-kappa">
          <?php
            $reply_text_combo = 'Reply<svg><use xlink:href="'. get_template_directory_uri() .'/img/symbols.svg#refresh"></use></svg>';
            comment_reply_link(
              array_merge(
                $args,
                array(
                  'reply_text' => $reply_text_combo,
                  'add_below' => $add_below,
                  'depth' => $depth,
